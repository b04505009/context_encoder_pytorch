...
class LSUNClass(VisionDataset):
    def __init__(
        ...
        self.root = root
        with lmdb.open(self.root, max_readers=1, readonly=True,
                             lock=False, readahead=False, meminit=False) as env:
            with env.begin(write=False) as txn:
                self.length = txn.stat()["entries"]

    def open_lmdb(self)-> None:
        self.env = lmdb.open(self.root, max_readers=1, readonly=True,
                             lock=False, readahead=False, meminit=False)
        with self.env.begin(write=False) as txn:
            self.length = txn.stat()["entries"]
        cache_file = "_cache_" + \
            "".join(c for c in self.root if c in string.ascii_letters)
...
class LSUN(VisionDataset):
    ...
    def __init__(
        ...
        for c in self.classes:
            self.dbs.append(LSUNClass(root=os.path.join(
                root, f"{c}_lmdb"), transform=transform))
